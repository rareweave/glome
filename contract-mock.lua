...
ADD_MODULE("functions/edit-nft", function()
...
ADD_MODULE("functions/finalize-buy", function()
 local function handle(state, action)
    ...
    print("listingAddress",state.listingAddress,fetchedTransferTx.to)
...
ADD_MODULE("functions/unlist", function()
 local function handle(state, action)
  assert(not state.reservationTxId or (SmartWeave.transaction.timestamp - state.reservationTimestamp) > 600000, "NFT is reserved for buy")
...
ADD_MODULE("functions/reserve-buying-zone", function()
 local function handle(state, action)
    ...
    local fetchedRoyaltyTx = SmartWeave.extensions[state.listingChain].readTxById(action.input.transferTxID):await()
    print("listingAddress",state.royaltyAddresses[state.listingChain],fetchedRoyaltyTx)
...
ADD_MODULE("functions/balance", function()
 local function handle(state, action)
...
ADD_MODULE("functions/list", function()
 local function handle(state, action)
...
ADD_MODULE("functions/transfer", function()
 local function handle(state, action)
...
local transfer = REQUIRE_MODULE "functions/transfer"
local list = REQUIRE_MODULE "functions/list"
local balance = REQUIRE_MODULE "functions/balance"
local reserveBuyingZone = REQUIRE_MODULE "functions/reserve-buying-zone"
local unlist = REQUIRE_MODULE "functions/unlist"
local finalizeBuy = REQUIRE_MODULE "functions/finalize-buy"
local editNft = REQUIRE_MODULE "functions/edit-nft"
local BNlib = REQUIRE_MODULE "bn"
BN=BNlib(256)
